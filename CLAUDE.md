...
> 프로젝트 원칙/컨벤션(redis-first, 트래픽 설계, 코딩 컨벤션 등)은 [CONSTITUTION.md](./CONSTITUTION.md)를 참조한다.
...
- **plan 모드의 읽기 전용 작업(파일 읽기, grep, 코드 탐색 등)은 모두 사전 승인된 것으로 간주한다.** 매번 확인하지 않고 자유롭게 조사할 것. 간단한 경우를 제외하고 AGENTS.md 파일을 참조하여 서브에이전트를 활용한다
