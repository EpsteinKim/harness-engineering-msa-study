...
interface SeatRepository : JpaRepository<Seat, Long> {
    ...
    @Query("SELECT COUNT(s) FROM Seat s WHERE s.eventId = :eventId AND s.status = com.epstein.practice.reserveservice.type.entity.SeatStatus.AVAILABLE")
    fun countAvailableSeats(@Param("eventId") eventId: Long): Long

    fun findByEventIdAndUserId(eventId: Long, userId: Long): Seat?
    fun findByEventIdAndUserIdAndStatus(eventId: Long, userId: Long, status: com.epstein.practice.reserveservice.type.entity.SeatStatus): Seat?

    @Query("""
        SELECT s FROM Seat s
        WHERE s.userId = :userId
          AND s.status IN (com.epstein.practice.reserveservice.type.entity.SeatStatus.PAYMENT_PENDING,
                           com.epstein.practice.reserveservice.type.entity.SeatStatus.RESERVED)
    """)
