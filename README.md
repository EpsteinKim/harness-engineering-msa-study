...
| reserve-service | 좌석 예약 + 이벤트별 대기열 + Redis 캐싱 + SEAT_PICK HOLD + Saga 오케스트레이터 | 8082 |
| user-service | 사용자 조회/수정 (인증 미도입) | 8081 |
| payment-service | 결제 처리 (학습용 랜덤 성공/실패) | 8083 |

### API

| Method | Path | 설명 |
|--------|------|------|
| GET | `/api/v1/reservations/events` | 이벤트 목록 (기본 status=OPEN, ticketOpenTime 오름차순) |
| GET | `/api/v1/reservations/events/{eventId}` | 이벤트 단건 (remainingSeats 캐시 기반) |
| GET | `/api/v1/reservations/my?userId=X` | 사용자 예약 이력 + payment-service 호출로 결제 상태 조합 |
| POST | `/api/v1/reservations` | 예약 요청 (seatId 또는 section) — userId 존재 검증 후 큐 진입 |
| POST | `/api/v1/reservations/pay` | 결제 saga 트리거 (PAYMENT_PENDING → RESERVED/AVAILABLE). 금액은 서버가 `seat.priceAmount`로 결정 |
...
| POST | `/api/v1/payments` | 결제 처리 (reserve-service가 내부 호출) |
| GET | `/api/v1/payments/{id}` | 결제 조회 |
| GET | `/api/v1/payments?userId=X` | 사용자 결제 이력 (reserve-service가 /my 조립용으로 호출) |
...
Client → POST /reservations (enqueue)
  ...
  → 배정 성공: Seat.status = PAYMENT_PENDING (RESERVED가 아님)
  → 유저가 POST /reservations/pay 호출 → PaymentOrchestrator
    → payment-service 호출 (RestClient 동기)
    → 성공: Seat.status = RESERVED
    → 실패 (Saga 보상): Seat.status = AVAILABLE, adjustSeatCounts(+1), markSeatAvailable, scheduler 재시작
  → 잔여석 0 시 큐 정리 + 스케줄러 중단 (cancel/결제실패로 잔여석 복구되면 자동 재시작)
```
...
| PAYMENT_PENDING_NOT_FOUND | 결제 대기 중인 좌석이 없음 (중복 pay 또는 미배정) |
| PAYMENT_FAILED | payment-service가 결제 거절 |
...
- payment-service + Saga 오케스트레이션 — PAYMENT_PENDING 중간 상태, 실패 시 Seat AVAILABLE 복구 + 스케줄러 재시작 보상
- 좌석/섹션별 가격 도입 + EventCache 가격 캐싱 — 클라 amount 위변조 차단, 응답에 priceAmount 노출
